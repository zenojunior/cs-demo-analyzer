 ...
 * `@cs2/replay-core/schema` format, along with player voice (comms). No demo
 * byte leaves the machine.
 */
import init, { parse_demo } from './parser/demo_parser.js'
import type { VoiceData } from '@cs2/replay-core/schema'
import { parseVoiceBlob } from '@cs2/replay-core/voiceCodec'

export interface ParseRequest {
  buffer: ArrayBuffer
  frameRate?: number
  /** Vendored copy only: explicit wasm URL (the extension bundles it via ?url,
   *  since `new URL(..., import.meta.url)` isn't emitted in the WXT build). */
  wasmUrl?: string
}
...
self.onmessage = async (e: MessageEvent<ParseRequest>) => {
  const { buffer, frameRate = 8, wasmUrl } = e.data
  try {
